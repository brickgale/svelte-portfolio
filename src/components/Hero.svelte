<script>
  ...
  const tech = [
    ...
    { name: "NodeJs", svg_name: "nodejs" },
    { name: "Laravel", svg_name: "laravel" },
    { name: "PHP", svg_name: "php" },
  ...
  onMount(() => {
    ...
    enableAnimations = perfLevel !== "low";
  });
...
<section
  id="about"
  class="min-h-screen w-full flex flex-col items-center justify-center py-20 px-5 relative"
>
  <h2 class="text-2xl md:text-3xl font-bold text-center" data-aos="fade-up" data-aos-delay="200">
    Hey there <span class:animate-wave={enableAnimations}>👋</span>
  </h2>
  <h1 class="text-2xl md:text-5xl text-center font-bold" data-aos="fade-up" data-aos-delay="400">
    I'm &nbsp;<span class="font-gothic text-3xl md:text-6xl text-(--ui-primary)"
      >Brian Monsales</span
    >
  </h1>
  <div
    class="text-header h-[40px] sm:h-[45px] md:h-[60px] overflow-hidden"
    data-aos="fade-up"
    data-aos-delay="500"
  >
    <WordRotate
      height="40"
      class="font-fira text-2xl sm:text-3xl md:text-4xl font-bold text-center mt-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 bg-clip-text text-transparent {enableAnimations
        ? 'text-shine'
        : ''}"
  ...
  <p
    class="mt-3 flex flex-row justify-center items-center gap-1 text-xs font-light tracking-wide px-4 py-2 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm transition-all duration-300 hover:bg-white/10 hover:border-white/20"
    data-aos="fade-up"
    data-aos-delay="600"
  >
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
    <div
      class="relative overflow-x-hidden py-4 md:py-8 infinite-scroll-container"
      data-aos="fade-up"
      data-aos-delay="700"
    >
  ...
  <img
    src={workstation.src}
    alt="workstation"
    class="absolute h-[400px] w-[820px] bottom-0 left-1/2 z-9 object-cover transform -translate-x-1/2"
    data-aos="fade-up"
    data-aos-delay="800"
  />
  <img
    src={programmer.src}
    alt="programmer"
    class="absolute max-h-[360px] w-full bottom-0 left-0 object-contain z-10"
    data-aos="fade-up"
    data-aos-delay="1200"
  />
</section>
