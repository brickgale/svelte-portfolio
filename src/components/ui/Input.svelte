...
<div class="relative w-full">
    {#if props.isTextarea}
        <textarea
            ...
            class="h-50 w-full cursor-default rounded-md border border-gray-800 bg-gray-950 p-3.5 text-gray-100 transition-colors duration-500 placeholder:select-none placeholder:text-gray-500 focus:border-(--ui-primary) focus:outline-none"
        ></textarea>
    {:else}
        <input
            onmousemove={handleMouseMove}
        ...
        <input
            ...
            aria-hidden="true"
            class="pointer-events-none absolute left-0 top-0 z-10 h-12 w-full cursor-default rounded-md border border-(--ui-primary) bg-transparent p-3.5 opacity-0 transition-opacity duration-500 placeholder:select-none"
        />
  {/if}
</div>