...
>
  <img
    ...
    placeholder="blur"
    alt={project.name}
    class="w-full h-full object-cover object-top m-0 transition-all ease-linear {isScrollable
  ...
  ></div>
  <div
    class="text-gray-100 sm:px-4 py-0 sm:pb-3 text-left absolute pb-2 px-4 md:pb-5 inset-x-0 bottom-0 flex flex-col justify-end size-full transform transition-transform duration-500 ease-out translate-y-4 group-hover:translate-y-0"
  >
    <h3
      class="text-lg font-bold tracking-wide mt-3 mb-1 transform transition-all duration-500 delay-75 translate-y-4 opacity-80 group-hover:translate-y-0 group-hover:opacity-100"
    >
      {project.name}
    </h3>
    <p
      class="min-h-[62px] text-sm leading-5 transform transition-all duration-500 delay-150 translate-y-6 opacity-70 group-hover:translate-y-0 group-hover:opacity-100"
    >
      {stringTruncate(project.description, 150)}
    </p>
  </div>
</div>
